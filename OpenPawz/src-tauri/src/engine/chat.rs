...
/// IDE tool allowlist — stored in Tauri managed state by host apps (OPIDE).
...
pub fn build_chat_tools(
    ...
    if let Some(assembler) = app_handle.try_state::<Box<dyn crate::atoms::traits::ToolAssembler>>() {
        ...
        // Legacy path: IdeToolAllowlist (will be removed once OPIDE migrates to ToolAssembler)
...
/// IDE-specific platform prompt — tells the agent it's running inside OPIDE.
...
/// IDE-specific coding guidelines — code generation rules for OPIDE.
...
/// Build the MCP/Foreman Protocol awareness block (OpenPawz chatbot mode).
///
/// Only used in OpenPawz sessions — OPIDE uses build_ide_mcp_prompt() instead.
/// Loaded from `prompts/foreman.md` at compile time.
pub fn build_foreman_awareness() -> &'static str {
    include_str!("prompts/foreman.md")
}

/// Build the IDE-specific MCP awareness block (OPIDE mode).
///
/// Replaces build_foreman_awareness() in OPIDE sessions. OPIDE agents don't
...
pub fn compose_chat_system_prompt(
    ...
    // In OPIDE: use IDE-specific prompts INSTEAD of generic OpenPawz platform.
    ...
    // Detect OPIDE mode: if IDE platform prompt is being used, we're in IDE mode
    let is_ide_mode = parts.iter().any(|p| p.contains("OPIDE") || p.contains("execute_code"));

    if is_ide_mode {
        // OPIDE mode — clean IDE-focused context
        parts.push(
            "## Memory\n\
        ...
        // OpenPawz mode — full soul file guidance
...
pub fn compose_chat_system_prompt_budgeted(
    ...
    let mut parts: Vec<String> = vec![
        ...
        // model instructions out of the OPIDE coding agent context.
